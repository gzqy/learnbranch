            ...
            "Ok": {
                text: '确定', "class": 'btn-done', click: function () {
                    ajax({
                        type: "POST",
                        dataType: 'json',
                        url: 'index.php?s=/Concat/deleteGroup',
                        data: "id=" + id,
                        success: function (rs) {
                            dialog.tip({
                                msg: rs.msg,
        ...
        success: function (data) {
            if (data.code == 200) {
                ...
                socket.onopen = function () {
                    ...
                    dialog.tip({
                        msg: "已经拨号，弹屏6秒后请拿起听筒！6秒后自动消失!<br/>" + data.data.phone_msg,
                        buttons: {
                            "Done": {
                                text: '确定', "class": 'btn-done', click: function () {
                                    dialog.hide();
                            ...
                            "Cancel": {
                                text: '取消拨号', "class": 'btn-done', click: function () {
                                    // var  socket1 = new WebSocket("ws://" + ws_address + ":1818");
                                    // line=1number=STOPcallOutId=12389522abc>
                                    socket.close();
                                    var  socket1 = new WebSocket("ws://" + ws_address + ":1818");
                                    var timestamp = (new Date()).valueOf();
                                    socket1.onpen=function() {
                                        socket1.send("line=" + ws_line + "number=STOPcallOutId=" + timestamp);
                                    }
                                }
                            }
                        }
                ...
                //         buttons: {
                //             "Done": {
                //                 text: '确定', "class": 'btn-done', click: function () { dialog.hide();}
                //             }
                //         }
                //     });
                //     // alert(msg.data);
