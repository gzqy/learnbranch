...
function getStatus(){
    ...
    ajax({
        ...
        success: function(rs){
            if ( rs.status == 200 ) {
                var On_html = '<span class="ui type label Online">On</span>';
                var Off_html = '<span class="ui type label Offline">Off</span>';
                $.each(rs.data,function(i,n){
                    ...
                    if ( n.cased ) {
                        $("._case_"+i).text(n.cased);
                    }
                    if ( n.CPU ) {
                        $("._CPU_"+i).css({'width':n.CPU+'%'}).attr('title',n.CPU+'%');
                        $("._CPU_"+i).parents(".progress").attr("class","ui progress "+n.CPU_CSS);
                    }
                    if ( n.Store ) {
                        $("._Store_"+i).css({'width':n.Store+'%'}).attr('title',n.Store+'%');
                        $("._Store_"+i).parents(".progress").attr("class","ui progress "+n.Store_CSS);
                    }
                    if ( n.Mem ) {
                        $("._Mem_"+i).css({'width':n.Mem+'%'}).attr('title',n.Mem+'%');
                        $("._Mem_"+i).parents(".progress").attr("class","ui progress "+n.Mem_CSS);
                    }
                    if ( n.screen_pops ) {
...
function getLineStatus(){
    ...
    ajax({
        ...
        success: function(html_rs){
            ...
            if ( rs.status == 200 ) {
                $.each(rs.data,function(i,n){
                    var html = '<a class="ui label"><img src="/status/'+n.case_icon+'" height="20" align="absmiddle" > '+n.case_text+'</a>';
