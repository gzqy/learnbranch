 ...
 * 设备控制器
 */
class DeviceController extends ShopbaseController {
    protected $nav_id = 7;
    ...
    public function showLine($id=null) {
        ...
        $con=['account_id'=>$this->account_id,
              'device_id'=>$id];
        //端口状态处理
        if($lines){
    ...
    public function showLogs($id=null) {
        ...
        $data = $this->deviceCall->getCallList($id);//通话记录列表
...
//        {fruitID:1,fruitName:"苹果"},
        ...
        $this->assign('recentDate',I('recent_date'));
...
//                            array('recording_time','通话时长'),
    ...
    public function saveScore(){
        if(IS_AJAX){
            ...
            $score = I('score');
            if(!$id || empty($score)){
                echo json_encode(array('status'=>300,'msg'=>'缺少参数'));exit;
            }
            $array = [1,2,3,4,5];
            if(!in_array($score, $array)){
                echo json_encode(array('status'=>300,'msg'=>'分数错误'));exit;
            }
            $call = M('DeviceCall')->where('id = '.$id)->find();
            if(!$call){
                echo json_encode(array('status'=>300,'msg'=>'记录错误'));exit;
            }
            $data['score'] = "{$score}";
            M('DeviceCall')->where('id='.$id)->save($data);
            echo json_encode(array('status'=>200,'msg'=>'记录评分成功'));exit;
        }else{
    ...
    function setLogAttention(){
        if(IS_AJAX){
            ...
            if(1 == $st){ //关注
                if($is){
                    echo json_encode(array('code'=>300,'msg'=>'记录已经关注'));exit;
                }else{
                    $time = time();
                    //处理
                    $data = array(
    ...
    function SavePortName(){
        if(IS_AJAX){
            $data = I('data');
            if(!$data){
                echo json_encode(array('code'=>300,'msg'=>'请填写参数'));exit;
            }
            if(!$data['device_id'] || !$data['code'] || !$data['PortName']){
                echo json_encode(array('code'=>300,'msg'=>'请填写端口名称'));exit;
            }
            //修改名称
            if(M('device_line')->where(array('device_id'=>$data['device_id'],'code'=>$data['code']))->save(array('PortName'=>$data['PortName']))){
                ...
                echo json_encode(array('code'=>200,'msg'=>'修改端口备注成功'));exit;
            }else{
                echo json_encode(array('code'=>300,'msg'=>'修改端口备注失败'));exit;
            }
        }else{
            echo json_encode(array('code'=>300,'msg'=>'请求方式错误'));exit;
        }
    ...
    function GetLineStatus(){
        if(IS_AJAX){
            $ids = I('ids');
            if(!$ids){
                echo json_encode(array('status'=>300,'msg'=>'没有请求内容'));exit;
            }
            foreach($ids as $k=>$v){
            ...
            echo json_encode(array('status'=>200,'data'=>$data));exit;
        }else{
            echo json_encode(array('status'=>300,'msg'=>'非法请求'));exit;
        }
    }
    ...
    function PlayLog(){
        ...
        $fileMp3 = str_replace(['.wav','.WAV'],['.mp3','.mp3'],$file);

        if(!file_exists($fileMp3) && strripos($file,'.wav')!==false){
    ...
    public function deleteLog(){
        $id = I('id','','int');
        if(!$id){
            echo json_encode(array('status'=>300,'msg'=>'请选择要删除的记录'));exit;
        }
        $log = M("device_call")->where("id = {$id}")->field("id,files")->find();
        if(!$log){
            echo json_encode(array('status'=>300,'msg'=>'记录不存在'));exit;
        }
        $is = D("Common/DeviceCall")->deleteLog($log);
        if($is){
            echo json_encode(array('status'=>200,'msg'=>'删除记录成功'));exit;
        }else{
            echo json_encode(array('status'=>300,'msg'=>'删除记录失败'));exit;
        }
    }
    ...
    public function uploadStatus(){
        ...
        $end_date = $end_date . ' 00:00:00'; $data = (array)$this->device->getDevicesUploadList();
...
//        $this->assign('status',$status ? $status : 0);
    ...
    public function updateRecord(){
        $data=I('');
        $needField=['note','admin_note','note_type'];
        $saveData=[];
        foreach ($needField as $v){
            if($data[$v]===null){
                continue;
            }
            if($v=='note' && $this->isAdmin){
//                $data[$v] = "[admin]{$data[$v]}[/admin]";
            }
            if($v=='admin_note' && !$this->isAdmin){
                continue;
            }
            $saveData[$v]=$data[$v];
        }

          M('device_app_call',null)->where(['id'=>(int)$_GET['id']])->save($saveData);
