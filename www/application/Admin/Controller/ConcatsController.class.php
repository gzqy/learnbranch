 ...
 * 客户资料板块
 */
class ConcatsController extends ShopbaseController
{
    protected $nav_id = 20;
    protected $second_nav = array();
    protected $notNeedAdmin = 0;
    // 全部是为了方便 下拉列表
    public static $feedbackTypeList = ['全部', '未选择意向', '有意向', '无意向', '拒绝'];
    function _initialize()
    {
        parent::_initialize();
        $second_nav = array(
            array('id' => 1, 'a' => U('/Concats/index'), 'name' => '客户管理'),
            array('id' => 2, 'a' => U('/Concats/GroupList'), 'name' => '客户类别'),
//            array('id' => 3, 'a' => U('/Concats/SourceList'), 'name' => '客户来源'),
//            array('id' => 3, 'a' => U('/Concat/statistics'), 'name' => '统计'),
    ...
    public function index()
    {
        ...
        $source_id = (int)I('source_id');
        $importantLevel= (int)I('important_level');
        $tel = I('tel');
        //where条件处理
        $name && $where['a.name'] = array('like', "%{$name}%");
        $tel1 && $where['a.tel1'] = array('like', "%{$tel1}%");
        $gender && $where['a.gender'] = $gender;
        $group_id && $where['a.app_group_id'] = $group_id;
        $importantLevel && $where['a.important_level'] = $importantLevel;
        $source_id && $where['a.source_id'] = $source_id;
        ...
        $isAdmin =$this->notNeedAdmin || $this->account_id == 10000000;
        if (!$isAdmin) {
            $form_is_dailed && $where['ct.is_dailed'] = $form_is_dailed;

            //分页处理

            $where['a.app_code'] = $_SESSION['ACCOUNTS']['tel'];
            $count = M("contacts")->alias('a')
                ->join("right join accounts as ct on ct.tel=a.app_code")
            ...
            $count = M("contacts")->alias('a')
                ...
                ->join("left join contact_group_app c on a.app_group_id = c.id")
                ->where($where)
                ->count();
        }
        $page = $this->page($count, $limit, array('source_id'=>$source_id,'important_level'=>$importantLevel,'tel'=>$tel,'p' => $p, 'limit' => $limit, "name" => $name, "tel1" => $tel1, "gender" => $gender, "group_id" => $group_id, "add_start_date" => $add_start_date, "add_end_date" => $add_end_date, 'upd_start_date' => $upd_start_date, 'upd_end_date' => $upd_end_date, 'form_feedback' => $feedbackTypeForm, 'form_is_dailed' => $form_is_dailed));
      
        if (!$isAdmin) {
            //查询数据

            $data = M("contacts")->alias("a")
                ->join("left join devices_app l on a.app_code= l.code")
                ->join("left join contact_stat b on a.id = b.contact_id")
                ->where($where)
                ->limit(($p-1)*$limit,$limit)
                ->field(" a.*,l.id device_id")
                ->group('tel1')
                ->order('id '.'desc')
            ...
            foreach ($data as $k=>$v){
                //联系人
                $detail = M("device_app_call")->alias("c")
                    ->where(['device_id'=>$v['device_id'],'tel'=>$v['tel1']])
                    ->field(" sum(case when c.type='10' then 1 else 0 end) as comeing,sum(case when c.type='9' then 1 else 0 end) as outgoing,sum(case when c.type='11' then 1 else 0 end) as missed")
                    ->select();
                $data[$k]['comeing']=$detail[0]['comeing'];
                $data[$k]['outgoing']=$detail[0]['outgoing'];
                $data[$k]['missed']=$detail[0]['missed'];
            ...
            $data = M("contacts")->alias('a')
                ->join("left join contact_stat b on a.id = b.contact_id")
                ->join("left join contact_group_app c on a.group_id = c.id")
                ->where($where)
                ->field("a.important_level,a.source_id,a.feedback_type,a.id,a.name,a.gender,a.tel1,a.group_id,a.add_time,a.upd_time,a.is_dailed,b.comeing,b.outgoing,b.missed,b.videod,c.name as gname,a.company,a.note")
                ->order("a.id desc")
                ->limit($page->firstRow . ',' . $page->listRows)
                ->select();
        }
...
//        $sourceList = array_column($sourceList,'name','id');
        //获取分组
//        $groups   = D("Common/ContactGroup")->getOption();//客户组
        $groups = M('contact_group_app',null)->where(['account_id'=>$this->account_id])->select();
...
//        $this->accountLogs->addLog("查看联系人列表，查询条件：联系人名称：{$name},联系人电话{$tel1},联系人分组id：{$group_id},联系人性别：{$gender},联系人注册时间最小值：{$add_start_date},联系人注册时间最大值：{$add_end_date},联系人更新最近时间：{$upd_end_date},联系人更新最远时间：{$upd_start_date}");
        ...
        $this->assign('feedbackType', self::$feedbackTypeList);
    ...
    public function tel()
    {
        ...
        if (!$isAdmin) {
            $form_is_dailed && $where['ct.is_dailed'] = $form_is_dailed;
            $where['ct.id'] = ['eq', $this->account_id];
            //分页处理
            $count = M("contacts")->alias('a')
        ...
        if (!$isAdmin) {
            ...
            $data = M("contacts")->alias('a')
                ...
                ->join("left join contact_group_app c on a.app_group_id = c.id")
                ->where($where)
                ->field("a.important_level,a.source_id,a.feedback_type,a.id,a.name,a.gender,a.tel1,a.group_id,a.add_time,a.upd_time,b.comeing,b.outgoing,b.missed,b.videod,c.name as gname")
                ->order("a.id desc")
                ->limit($page->firstRow . ',' . $page->listRows)
                ->select();
            ...
            $data = M("contacts")->alias('a')
                ...
                ->join("left join contact_group_app c on a.group_id = c.id")
                ->where($where)
                ->field("a.important_level,a.source_id,a.feedback_type,a.id,a.name,a.gender,a.tel1,a.group_id,a.add_time,a.upd_time,a.is_dailed,b.comeing,b.outgoing,b.missed,b.videod,c.name as gname")
                ->order("a.id desc")
                ->limit($page->firstRow . ',' . $page->listRows)
                ->select();
        }
...
//        $this->accountLogs->addLog("查看联系人列表，查询条件：联系人名称：{$name},联系人电话{$tel1},联系人分组id：{$group_id},联系人性别：{$gender},联系人注册时间最小值：{$add_start_date},联系人注册时间最大值：{$add_end_date},联系人更新最近时间：{$upd_end_date},联系人更新最远时间：{$upd_start_date}");
    ...
    public function add()
    {
        ...
        if ($id) { //编辑
            ...
            if(!$_SESSION['ACCOUNTS']['tel']){

                $this->error("当前账户不能添加联系人");
    ...
    public function addCustomer(){
        $data=I('');
        $code = $data['id'];
        $data['app_code']=$_SESSION['ACCOUNTS']['tel'];
        if(empty($_SESSION['ACCOUNTS']['tel'])){
            exit(json_encode(['code'=>'0001','msg'=>'当前账户手机号不能为空']));
        }
...
//            where(['app_code'=>$code,'name'=>$data['group']])->getField('id');
        ...
        exit(json_encode(['code'=>'0000','msg'=>'手机号操作成功']));
    }
    public function save()
    {
        if (IS_AJAX) {
            ...
            if ($data['id']) { //编辑联系人
                //判断id是否正确
                if (!($record = M('contacts')->find($data['id']))) {
                ...
                $ids = M('contacts',null)->where([
                    'tel1'=>$data['tel1'],
                ])->field('id')->select();
                if($ids){
                    $ids=array_column($ids,'id');
                    $exist=M('contacts_to_accounts',null)->where([
                        'account_id'=>$this->account,
                        'contact_id'=>['in',$ids]
                    ])->select();
                    if($exist){
                        echo json_encode(array('code' => 300, 'msg' => '联系号码:'.$data['tel1'].'已存在'));
                        exit;
                    }
                }
                $data['app_code']=$_SESSION['ACCOUNTS']['tel'];
                ...
                $data['keywords'] = D("Common/Contact")->getKeywords($data);
                if(!$data['note_list']){
                    $data['note_list'] =date("Y-m-d H:i:s") . "\n" . $data['note'];
                }
                ...
                if ($id = M("contacts")->add($data)) {
                    ...
                    $r['contact_id'] = (int)$id;
                    $r['account_id'] = $this->account_id;
                    $r['add_time'] = time();
                    $r['account_id'] = $this->account_id;
                    $r['group_id']=$data['group_id'];
                    M('contacts_to_accounts', null)->add($r);
...
//        }
//        $this->accountLogs->addLog("查询客户组，查询条件：分组名称：{$name},添加最小时间：{$add_time_min},添加最大时间：{$add_time_max},修改最小时间：{$upd_time_min},修改最大时间：{$upd_time_max}");
//        $this->assign('name', $name);
...
//    }
    ...
    function GroupList()
    {
        ...
        //分页处理
        $count = M("contact_group_app")->where($where)->count();
        $page  = $this->page($count, $limit, array('p' => $p, "name" => $name, "add_time_min" => $add_time_min, "add_time_max" => $add_time_max, "upd_time_min" => $upd_time_min, "upd_time_max" => $upd_time_max));
        //查询列表
        $data = M("contact_group_app")->where($where)->order("id desc")->limit($page->firstRow . ',' . $page->listRows)->select();
        //统计查询每个分组的设备数量
        foreach ($data as $k => $v) {
            $data[$k]['num'] = M("contacts")->where("group_id = {$v['id']}")->count();
        }
...
//
//        $code=$_SESSION['ACCOUNTS']['tel'];
//        $data=M('contact_group_app')->where(['app_code'=>$code])->select();
//        $this->assign('data', $data);
//    }
    ...
    function SourceList()
    {
        ...
        //分页处理
        $count = M("contact_source")->where($where)->count();
        $page  = $this->page($count, $limit, array('p' => $p, "name" => $name, "add_time_min" => $add_time_min, "add_time_max" => $add_time_max, "upd_time_min" => $upd_time_min, "upd_time_max" => $upd_time_max));
        //查询列表
        $data = M("contact_source")->where($where)->order("id desc")->limit($page->firstRow . ',' . $page->listRows)->select();
        //统计查询每个分组的设备数量
        foreach ($data as $k => $v) {
            $data[$k]['num'] = M("contacts")->where("source_id= {$v['id']}")->count();
        }
//        $this->accountLogs->addLog("查询客户组，查询条件：分组名称：{$name},添加最小时间：{$add_time_min},添加最大时间：{$add_time_max},修改最小时间：{$upd_time_min},修改最大时间：{$upd_time_max}");
        $this->assign('name', $name);
        $this->assign('add_time_min', $add_time_min);
    ...
    function addGroup()
    {
        $id = I('id');
        if ($id) { //编辑
            $group = M("contact_group_app")->find($id);
            if (!$group) {
                $this->error("客户组不存在");
            }
    ...
    function addSource()
    {
        $id = I('id');
        if ($id) { //编辑
            $group = M("contact_source")->find($id);
            if (!$group) {
                $this->error("来源不存在");
            }
            $this->assign('data', $group);
...
//                    $this->accountLogs->addLog("编辑客户组，组id：{$data['id']}");
...
//                        $this->accountLogs->addLog("添加客户组，组id：{$id}");
...
//    }
    ...
    public function saveGroup(){
        $data = I('data');
        $dates=(['app_code'=>$_SESSION['ACCOUNTS']['tel'],'name'=>$data['name']]);
        if($this->account_id == 10000000){
            exit(json_encode(['code'=>'0001','msg'=>'管理员不能添加分组']));
        }
        if(empty($_SESSION['ACCOUNTS']['tel'])    ){
            exit(json_encode(['code'=>'0001','msg'=>'当前登录用户手机号不能为空']));
        }
        if(empty($data['name'])){
            echo json_encode(array('code' => 300, 'msg' => '分组名称不能为空'));

        }

        if ($data['id']) { //编辑
                if (!M('contact_group_app')->find($data['id'])) {
                    echo json_encode(array('code' => 300, 'msg' => '分组不存在'));
                    exit;
                }
                $data['upd_time'] = date("Y-m-d H:i:s", time());
                if (M("contact_group_app")->save($data)) {
                    $this->accountLogs->addLog("编辑客户组，组id：{$data['id']}");
                    echo json_encode(array('code' => 200, 'msg' => '编辑分组成功'));
                    exit;
                } else {
            ...
            $appGroupId=M('contact_group_app',null)->
            where(['app_code'=>$_SESSION['ACCOUNTS']['tel'],'name'=>$data['name']])->getField('id');

            if($appGroupId){
                exit(json_encode(['code'=>'0000','msg'=>'分组名称已经添加']));
            }
            $dates['add_time'] = date("Y-m-d H:i:s", time());
            M('contact_group_app')->add($dates);
            exit(json_encode(['code'=>'0000','msg'=>'分组名称已经添加']));
        }
...
//    }
    ...
    function saveSource()
    {
        if (IS_AJAX) {
            $data = I('data');
            if ($data['id']) { //编辑
                if (!M('contact_source')->find($data['id'])) {
                    echo json_encode(array('code' => 300, 'msg' => '来源不存在'));
                    exit;
                }
                $data['upd_time'] = date("Y-m-d H:i:s", time());
                if (M("contact_source")->save($data)) {
//                    $this->accountLogs->addLog("编辑客户组，组id：{$data['id']}");
                    echo json_encode(array('code' => 200, 'msg' => '编辑成功'));
                    exit;
                } else {
                    echo json_encode(array('code' => 200, 'msg' => '编辑失败'));
                    exit;
                }
            } else { //添加
                if (M("contact_source")->where(array("name" => $data['name']))->find()) {
                    echo json_encode(array('code' => 200, 'msg' => '已经存在'));
                    exit;
                } else {
                    //添加分组
                    $data['add_time'] = date("Y-m-d H:i:s", time());
                    $data['upd_time'] = date("Y-m-d H:i:s", time());
                    if ($id = M("contact_source")->add($data)) {
//                        $this->accountLogs->addLog("添加客户组，组id：{$id}");
    ...
    function deleteGroup()
    {
        if (IS_AJAX) {
            ...
            if (!M("contact_group_app")->find($id)) {
            ...
            //删除分组
            if (M("contact_group_app")->delete($id)) {
                $this->accountLogs->addLog("删除客户组，组id：{$id}");
    ...
    function deleteSource()
    {
        if (IS_AJAX) {
            $id = I('id', 'int');
            if (!M("contact_source")->find($id)) {
                echo json_encode(array('code' => 300, 'msg' => '来源不存在'));
                exit;
            }
            //进行删除
            //首先将本分组下的联系人分组更换为0
            M("contacts")->where("source_id= {$id}")->save(array("source_id" => 0));
            //删除分组
            if (M("contact_source")->delete($id)) {
//                $this->accountLogs->addLog("删除客户组，组id：{$id}");
                echo json_encode(array('code' => 200, 'msg' => '删除成功'));
                exit;
            } else {
                echo json_encode(array('code' => 300, 'msg' => '删除失败'));
                exit;
            }
        } else {
            echo json_encode(array('code' => 300, 'msg' => '请求方式错误'));
    ...
    function import_excel()
    {
        ...
        for ($i = 2; $i <= $highestRow; $i++) {
            ...
            $data['city']   = $objPHPExcel->getActiveSheet()->getCell("D" . $i)->getValue();
            ...
            $data['note']   = $objPHPExcel->getActiveSheet()->getCell("H" . $i)->getValue();
            $data['app_code']   =$account['tel'];
            $r[]              = $data;
        }
...
//        $existsTels = M('contacts', null)->field('tel1')->where(['tel1' => ['in', array_column($r, 'tel1')]])->select();
        $existsTels = M('contacts', null)->alias('a')->field('a.tel1')->
        join('contacts_to_accounts as b on a.id=b.contact_id')->
        where(['a.tel1' => $data['tel1'],
        ...
        $firstId= M('contacts', null)->addAll($r);
        $lastId=$firstId+count($r)-1;
        $data=[];
        $time=time();
    ...
    public function telToAccount()
    {
        ...
        if ($this->notNeedAdmin || $this->account_id != 10000000) {
        ...
        foreach ($ids as $v) {
            ...
            if ($param['del']) {
                M('contacts_to_accounts',null)->where(['account_id'=>$r['account_id'],'contact_id'=>$r['contact_id']])->delete();
            }
            if(empty($param['del']) && !M('contacts_to_accounts', null)->
                where(['contact_id'=>$r['contact_id'],'account_id'=>$r['account_id']])->find() ){
                M('contacts_to_accounts',null)->add($r);
            }
        }
