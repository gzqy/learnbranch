...
class AppController extends Controller{
    ...
    public function Event(){
        ...
        if (empty($device_code)  || empty($event_type)) {
        ...
        $model->save();
        //进行添加
        $id=M('contacts')->where(['tel1'=>$caller,'app_code'=>$device_code,'app_delete'=>0])
            ->getField('id');
        ...
        $data['note']=I('note');
        $data['record_stop_time']=I('record_stop_time');
        $data['record_start_time']=I('record_start_time');
        ...
        if (1==1) {
            $log=date('Y-m-d,H:i:s',time()).json_encode($data).'<hr>';
            $filename = "./public/checkForlog.log";
...
//                }
    ...
    public function shardFiles(){
        ...
        if(!$exist){
            ...
            $appendData=['addtime'=>date('Y-m-d H:i:s'),
			 ...
			 file_put_contents('./public/testcon.log',$shard_content);
    ...
    public function recordList(){
        ...
        $start_call_time && $where['a.call_date']=['egt',$start_call_time];
        $end_call_time && $where['a.call_date']=['elt',$end_call_time];
        $min_recording && $where['a.recording_time']=['egt',$min_recording];
        $max_recording && $where['a.recording_time']=['elt',$max_recording];
//        $tel&&$where['tel']=['like','%'.$tel.'%'];
        $tel&&$where['tel']=['eq',$tel];
		if($search_type=='60'){
			$search_type && $where['type']=['in',$search_type];
		}else{
		  //	$where['_string']=" b.code=c.app_code";
			$search_type && $where['type']=['in',$search_type];
		}
		...
		$count = M("device_app_call")->alias("a")
            ...
            ->where($where)
            ->field("a.id,a.line_id,a.type,a.add_time,call_time,a.tel,a.rings_number,b.PortName")
            ->count();
...
//         
    ...
    public function updateLastTime(){
		...
		file_put_contents('./public/test.log',date().M()->_sql().'<br/>'.$deviceId.'-'.time());
	...
	public function logs($content){
        file_put_contents('./public/test.log',date('Y-m-d H:i:s',time()).$content.'<br/>');
    }
