...
class MenuModel extends CommonModel {
     ...
     * @param integer $with_self  是否包括他自己
     */
    public function shop_menu($parentid, $with_self = false,$AppName = 'Shop') {
        //父节点ID
        $parentid = (int) $parentid;

        $where['parentid'] = $parentid;
        $where['status'] = 1;
        $where['_string'] = 'app="Portal" OR app="' . $AppName . '"';
     ...
     * @param $parentid 菜单id
     */
    public function shopSubmenu($parentid = '', $big_menu = false,$appName = 'Shop') {
        $array = $this->shop_menu($parentid, 1, $appName);
        $numbers = count($array);
        if ($numbers == 1 && !$big_menu) {
            return '';
        }
     ...
     * 菜单树状结构集合
     */
    public function menu_shop_json($appName) {
        $data = $this->get_shop_tree(0,"",1,$appName);
        return $data;
    }
    
    //取得树形结构的菜单
    public function get_shop_tree($myid, $parent = "", $Level = 1,$appName = 'Shop') {
        $data = $this->shop_menu($myid,false,$appName);
        $Level++;
        if (is_array($data)) {
            foreach ($data as $a) {
                $id = $a['id'];
                ...
                $ret[$id . $name] = $array;
                $child = $this->get_shop_tree($a['id'], $id, $Level,$appName);
                //由于后台管理界面只支持三层，超出的不层级的不显示
                if ($child && $Level <= 3) {
                    $ret[$id . $name]['items'] = $child;
                }
     ...
     * @param number $parentid
     */
    public function get_menu_shop_tree($parentid=0){
        $menus=$this->where(array("parentid"=>$parentid))->order(array("listorder"=>"ASC"))->select();
